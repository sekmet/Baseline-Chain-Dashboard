...
const verifierContract = require("../artifacts/VerifierNoop.json");
...
describe("Error checks", () => {
  ...
  test("baseline_verifyAndPush fails when tree is full", async () => {
    ...
    if (txManager === 'besu') {
      ...
      //console.log("BODY ==>> ", res_2);
      expect(res_2.statusCode).toEqual(200);
      //expect(res_2.body.error).not.toBeUndefined();
      //expect(res_2.body.error.message).toEqual("VM Exception while processing transaction: revert There is no space left in the tree.");
      expect(res_2.body.error).toBeUndefined();
      //expect(res_2.body.result).toBeNull();
      expect(res_2.body.result).not.toBeUndefined();
